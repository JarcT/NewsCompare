...
const screen = async() => {
    ...
    for (let i = 0; i < names.length; i++){ 
        ...
        if(!archive){
            await ArchiveByName.create({name: name, oldScreenshot: newScreenshot})
        }
        if(archive){
            const {oldScreenshot} = archive 
            await ArchiveByName.findOneAndUpdate({name: archive.name}, {oldScreenshot: [...oldScreenshot, newScreenshot]})
        }
        //-------------archive by name
        ...
        const image = await screenShot(currentWebsite.name, currentWebsite.url, currentWebsite.cookieSelector) 
        //updates current database
        await DailyScreenshots.findOneAndUpdate({name:names[i]}, {newScreenshot: image.secure_url}) 
    }
    //--------------- archive by date
    const date = new Date()
